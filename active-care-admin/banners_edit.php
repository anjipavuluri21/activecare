...
{	
	...
	$login_query = "UPDATE activecare_banners SET `title`='$title','content'='$content' WHERE id=$id";
	...
	{			
		...
		if (($ext == "jpg") || ($ext == "jpeg") || ($ext == "png")) {
			...
			if (move_uploaded_file($_FILES['thumbimg1']['tmp_name'], $target_file1)) {	
				$login_query = "UPDATE activecare_banners SET thumbimg1='$basename1' WHERE id=$id";
				$database1 = new Database();
				$dbCon1 = $database1->getConnection();
				$stmt1 = $dbCon1->prepare($login_query);  
				$stmt1->execute();
	...
	if($img_location!="")
	{		
		$login_query = "UPDATE activecare_banners SET thumimage1='$img' WHERE id=$id";
		$database1 = new Database();
		$dbCon1 = $database1->getConnection();
		$stmt1 = $dbCon1->prepare($login_query);  
		$stmt1->execute();
	...
	for($p=0;$p<$countprods;$p++){				
		...
		if($name1!=""){						
			if (($extension1 == "jpg") || ($extension1 == "jpeg") || ($extension1 == "png")|| ($extension1 == "mp4")) {								
				$newfilename1 = 'active-care-banners-'.$p.rand(). round(microtime(true));			
	...
	if($res == 1 || $res1 == 1|| $res11 == 1|| $res12 == 1){
		...
		header("location:banners_edit.php?msg=fail");		
	}	
}	
?>
<style>
...
<link rel="stylesheet" href="main.css">
        ...
        <div class="right_col" role="main">
          <div class="">
			...
			<?php
				$banner_que = "SELECT * from activecare_banners where id=$id";
				$database1 = new Database();
				$dbCon1 = $database1->getConnection();
                    ...
                    <form action="" name="adv_form" id="adv_form" method="post" enctype="multipart/form-data" data-parsley-validate class="form-horizontal form-label-left">
					  ...
					  <div class="form-group">
						...
						<div class="col-md-9 col-sm-9 col-xs-12">
							<input type="text" name="title" id="name" required="required" value="<?php echo $about_res['title'] ?>" class="form-control" style="width: 780px;">
						</div>
                      </div>
                        <div class="form-group">
						 ...
						 <div class="control-group">					
							<div class="controls">
								...
								<?php if($about_res['thumbimg1']!=""){ ?>
                                                                    <img src="../uploads/banners/<?php echo $about_res['thumbimg1'] ?>" width="250px" height="150px"/>
								<?php } ?>		
								</div>	
							</div>
						 </div>						  
