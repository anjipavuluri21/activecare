...
{	
	...
	$login_query = "UPDATE activecare_ourteam SET `title`='$title',`content`='$content',`name`='$name' WHERE id=$id";
	...
	{			
		...
		$newfilename = 'active-care-ourteam-'. round(microtime(true));
		if (($ext == "jpg") || ($ext == "jpeg") || ($ext == "png")) {
			$basename1 = $newfilename .'.'. $extension1;
			$blocation1 = "../uploads/ourteam/" . $basename1;
			$target_file1 = "../uploads/ourteam/" . $basename1;
			if (move_uploaded_file($_FILES['thumbimg1']['tmp_name'], $target_file1)) {	
				$login_query = "UPDATE activecare_ourteam SET thumbimg1='$basename1' WHERE id=$id";
				$database1 = new Database();
				$dbCon1 = $database1->getConnection();
				$stmt1 = $dbCon1->prepare($login_query);  
				$stmt1->execute();
	...
	if($img_location!="")
	{		
		$login_query = "UPDATE activecare_ourteam SET thumimage1='$img' WHERE id=$id";
		$database1 = new Database();
		$dbCon1 = $database1->getConnection();
		$stmt1 = $dbCon1->prepare($login_query);  
		$stmt1->execute();
	...
	for($p=0;$p<$countprods;$p++){				
		...
		if($name1!=""){						
			if (($extension1 == "jpg") || ($extension1 == "jpeg") || ($extension1 == "png")|| ($extension1 == "mp4")) {								
				$newfilename1 = 'active-care-ourteam-'.$p.rand(). round(microtime(true));			
	...
	if($res == 1 || $res1 == 1|| $res11 == 1|| $res12 == 1){
		header("location:ourteam.php?msg=success");		
	} else {		
		header("location:ourteam_edit.php?msg=fail");		
	}	
}	
?>
<style>
...
<link rel="stylesheet" href="main.css">
        ...
        <div class="right_col" role="main">
          <div class="">
			...
			<?php
				$banner_que = "SELECT * from activecare_ourteam where id=$id";
				$database1 = new Database();
				$dbCon1 = $database1->getConnection();
                    ...
                    <form action="" name="adv_form" id="adv_form" method="post" enctype="multipart/form-data" data-parsley-validate class="form-horizontal form-label-left">
					  ...
					  <div class="form-group">
				       ...
				       <div class="form-group">
						 ...
						 <div class="control-group">					
							<div class="controls">
								...
								<?php if($about_res['thumbimg1']!=""){ ?>
                                                                    <img src="../uploads/ourteam/<?php echo $about_res['thumbimg1'] ?>" width="250px" height="150px"/>
								<?php } ?>		
								</div>	
							</div>
						 </div>						  
                        ...
                        <div class="form-group">
                        <label class="control-label col-md-3 col-sm-3 col-xs-12" for="last-name">Profile Name
                        </label>                        
						<div class="col-md-9 col-sm-9 col-xs-12">
							<input type="text" name="name" id="name" value="<?php echo $about_res['name'] ?>" class="form-control" style="width: 780px;">
						</div>
                      </div>
