...
{			
	...
	if($npassword==$cpassword){		
		$login_query = "SELECT * from activecare_adminuser where id=$loginid";
		...
		foreach($menbanner_res2 as $login_res) {								
			...
			{				
				$login_query2 = "UPDATE activecare_adminuser SET password='$cpassword' where id=$loginid";
				$database = new Database();
				$dbCon = $database->getConnection();
	...
	{			
		...
		if (($ext == "jpg") || ($ext == "jpeg") || ($ext == "png")) {			
			...
			if (move_uploaded_file($_FILES['image1']['tmp_name'], $target_file1)) {					
				$login_query = "UPDATE activecare_adminuser SET profilepic='$basename1' WHERE id=$loginid";
				$database = new Database();
				$dbCon = $database->getConnection();
				$stmt = $dbCon->prepare($login_query);  
				$stmt->execute();
				  ...
				  <div class="control-group">					
						<div class="controls">
							...
							<?php if($login_res['profilepic']!=""){ ?>
                                                            <img src="../uploads/userprofiles/<?php echo $login_res['profilepic'] ?>" width="250px" height="150px"/>
							<?php } ?>										
							</div>	
						</div>
					</div>
