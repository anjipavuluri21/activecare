...
if (isset($_POST['submit'])) {
    ...
    $login_query = "UPDATE activecare_products SET `title`='$name',`price`='$price',`content`='$content' WHERE id=$id";
    ...
    if ($thumbimg1 != "") {
        ...
        if (($ext == "jpg") || ($ext == "jpeg") || ($ext == "png")) {
            ...
            if (move_uploaded_file($_FILES['thumbimg1']['tmp_name'], $target_file1)) {
                $login_query = "UPDATE activecare_products SET thumimage1='$basename1' WHERE id=$id";
                $database1 = new Database();
                $dbCon1 = $database1->getConnection();
                $stmt1 = $dbCon1->prepare($login_query);
                $stmt1->execute();
    ...
    if ($img_location != "") {
        $login_query = "UPDATE activecare_products SET thumimage1='$img' WHERE id=$id";
        $database1 = new Database();
        $dbCon1 = $database1->getConnection();
        $stmt1 = $dbCon1->prepare($login_query);
        $stmt1->execute();
    ...
    if ($thumbimg2 != "") {
        ...
        if (($ext == "pdf")) {
            ...
            if (move_uploaded_file($_FILES['thumbimg2']['tmp_name'], $target_file1)) {
                $login_query = "UPDATE activecare_products SET thumimage2='$basename1' WHERE id=$id";
                $database1 = new Database();
                $dbCon1 = $database1->getConnection();
                $stmt1 = $dbCon1->prepare($login_query);
                $stmt1->execute();
        ...
        if ($name1 != "") {
            if (($extension1 == "jpg") || ($extension1 == "jpeg") || ($extension1 == "png") || ($extension1 == "mp4")) {
                ...
                if (move_uploaded_file($_FILES['prodimage']['tmp_name'][$p], $target_file1)) {
                    $login_query = "INSERT INTO activecare_prodimages (`prodid`,`image`) VALUES ('$id','$blocation1')";
                    $database1 = new Database();
                    $dbCon1 = $database1->getConnection();
                    $stmt1 = $dbCon1->prepare($login_query);
                    $stmt1->execute();
...
<?php
$banner_que = "SELECT * from activecare_products where id=$id";
$database1 = new Database();
$dbCon1 = $database1->getConnection();
                    ...
                    <div class="x_content">
                        ...
                        <form action="" name="adv_form" id="adv_form" method="post" enctype="multipart/form-data" data-parsley-validate class="form-horizontal form-label-left">
                            ...
                            <div class="form-group">
                                <label class="control-label col-md-3 col-sm-3 col-xs-12" for="last-name">Book Title<span class="required">*</span>
                                </label>                        
                                <div class="col-md-9 col-sm-9 col-xs-12">
                                    <input type="text" name="name" id="name" required="required" value="<?php echo $about_res['title'] ?>" class="form-control" style="width: 780px;">
                                </div>
                            </div>	
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-md-9 col-sm-9 col-xs-12">
                                    <input type="text" name="price" id="name" required="required" value="<?php echo $about_res['price'] ?>" class="form-control" style="width: 780px;">
                                </div>
                            </div>		 
                            <div class="form-group">
                                <label class="control-label col-md-3 col-sm-3 col-xs-12" for="first-name"> Book Image<span class="required">*</span>
                                </label>
