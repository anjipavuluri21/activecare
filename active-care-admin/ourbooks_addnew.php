...
{		
	...
	$login_query = "INSERT INTO activecare_products (`title`,`price`,`content`,`thumimage1`,`thumimage2`) VALUES ('$name', '$price', '$content','$thumbimg1', '$thumbimg2')"; 
	...
	{			
		...
		if (($ext == "jpg") || ($ext == "jpeg") || ($ext == "png")) {
			...
			if (move_uploaded_file($_FILES['thumbimg1']['tmp_name'], $target_file1)) {	
				$login_query = "UPDATE activecare_products SET thumimage1='$blocation1' WHERE id=$lastinsertid";
				$database1 = new Database();
				$dbCon1 = $database1->getConnection();
				$stmt1 = $dbCon1->prepare($login_query);  
				$stmt1->execute();
	...
	{			
		...
		if (($ext == "pdf")) {
			...
			if (move_uploaded_file($_FILES['thumbimg2']['tmp_name'], $target_file1)) {	
				$login_query = "UPDATE activecare_products SET thumimage2='$blocation1' WHERE id=$lastinsertid";
				$database1 = new Database();
				$dbCon1 = $database1->getConnection();
				$stmt1 = $dbCon1->prepare($login_query);  
				$stmt1->execute();
        ...
        <div class="right_col" role="main">
          <div class="">
			...
			<div class="form-group">
						...
						<div class="col-md-9 col-sm-9 col-xs-12">
							<input type="text" name="price" id="name" required="required" value="" class="form-control" style="width: 780px;">
						</div>
                      </div>		 
                        <div class="form-group">
