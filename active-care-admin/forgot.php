...
if(isset($_POST['Submit']))
{
	...
	if($password != $confpassword)
	{
		...
		$login_query = "SELECT * from activecare_adminuser where status=1 AND username='$username'";
		...
		{		
			...
			if($username==$db_username)
			{
				...
				$login_query = "UPDATE activecare_adminuser SET password='$newpass' where status=1 AND username='$username'";
				$database = new Database();
				$dbCon = $database->getConnection();
				$stmt = $dbCon->prepare($login_query);  
				$stmt->execute();
