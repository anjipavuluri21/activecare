	...
	{			
		...
		if (($ext == "jpg") || ($ext == "jpeg") || ($ext == "png")) {
			...
			if (move_uploaded_file($_FILES['thumbimg1']['tmp_name'], $target_file1)) {	
				$login_query = "UPDATE activecare_products SET thumimage1='$basename1' WHERE id=$lastinsertid";
				$database1 = new Database();
				$dbCon1 = $database1->getConnection();
				$stmt1 = $dbCon1->prepare($login_query);  
				$stmt1->execute();
	...
	{			
		...
		if (($ext == "pdf")) {
			...
			if (move_uploaded_file($_FILES['thumbimg2']['tmp_name'], $target_file1)) {	
				$login_query = "UPDATE activecare_products SET thumimage2='$basename1' WHERE id=$lastinsertid";
				$database1 = new Database();
				$dbCon1 = $database1->getConnection();
				$stmt1 = $dbCon1->prepare($login_query);  
				$stmt1->execute();
